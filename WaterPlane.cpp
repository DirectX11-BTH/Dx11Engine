...
Mesh WaterPlane::waterGeometry;
float WaterPlane::scaling = 100.f;


void WaterPlane::generateWaterPlane()
{
	waterGeometry.ambientMeshColor = DirectX::XMFLOAT4(100 / 255, 100 / 255, 100 / 255, 100 / 255);
	waterGeometry.diffuseMeshColor = DirectX::XMFLOAT4(20 / 255, 100 / 255, 100 / 255, 100 / 255);
	DirectX::XMFLOAT4 specularMeshColor = DirectX::XMFLOAT4(100 / 255, 100 / 255, 100 / 255, 100 / 255);

	float sizeMultiplier = scaling;

	for (int y = 0; y < quadsPerAxis; y++)
	{
		for (int x = 0; x < quadsPerAxis; x++)
		{
			...
			{
				waterGeometry.vertices.push_back(vertList.at(i));
			}
		}
	}

	DxHandler::createVertexBuffer(waterGeometry);
	std::cout << "Nr of verts for water:" << (waterGeometry.vertices.size() / 3) / 2 << std::endl;;
}

void WaterPlane::readTexture(std::string textureName)
{
}

std::vector<Vertex> WaterPlane::generateQuad(XMFLOAT3 corner0, XMFLOAT3 corner1, XMFLOAT3 corner2, XMFLOAT3 corner3)
{
	...
	*/
	...
	vert0.y = corner0.y;
	vert0.z = corner0.z;
	vert0.u = corner0.x / (quadsPerAxis ); //ing);;
	vert0.v = corner0.z / (quadsPerAxis ); //ing);;
	...
	vert3.y = corner1.y;
	vert3.z = corner1.z;
	vert3.u = corner1.x / (quadsPerAxis ); //ing);
	vert3.v = corner1.z / (quadsPerAxis ); //ing);;
	...
	vert4.y = corner3.y;
	vert4.z = corner3.z;
	vert4.u = corner3.x / (quadsPerAxis ); //ing);;
	vert4.v = corner3.z / (quadsPerAxis ); //ing);;
	...
	vert5.y = corner2.y;
	vert5.z = corner2.z;
	vert5.u = corner2.x / (quadsPerAxis);// ); //ing);;
	vert5.v = corner2.z / (quadsPerAxis);// ); //ing);;
